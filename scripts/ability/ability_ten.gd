...
func _throw_bomb(direction: Vector2):
	...
	current_bomb.damage_component.amount = int(impact_damage * current_parent.modifiers.gett(Player.Modifiers.DAMAGE))
...
func _spawn_explosion():
	...
	current_explosion.damage_component.amount = int(explosion_damage * current_parent.modifiers.gett(Player.Modifiers.DAMAGE))
...
func _spawn_all_projectiles():
	...
	for added_rotation in [0, 90, 180, 270]:
		...
		projectile.damage_component.amount = int(projectile_damage * current_parent.modifiers.gett(Player.Modifiers.DAMAGE))
